<script lang="ts">
	...
	let {
		...
		isTeacher,
		onEnrollment,
		showEnrollmentButtons = false,
		showTeacherActions = false,
		enrollmentLoading = null
	} = $props<{
		clases: DTOClase[];
		isEnrolled: (clase: DTOClase) => boolean;
		isTeacher: (clase: DTOClase) => boolean;
		onEnrollment: (claseId: number, enroll: boolean) => void;
		showEnrollmentButtons?: boolean;
		...
		<tbody class="divide-y divide-gray-200 bg-white">
			{#each clases as clase}
				<tr class="hover:bg-gray-50">
					...
					<td class="px-6 py-4 text-sm whitespace-nowrap text-gray-900">
						{clase.numeroAlumnos || 0}
					</td>
					<td class="px-6 py-4 text-sm font-medium whitespace-nowrap">
						<div class="flex space-x-2">
							{#if showEnrollmentButtons}
								{#if enrollmentLoading === clase.id}
									<button
										disabled
										class="rounded bg-gray-50 px-2 py-1 text-xs text-gray-400 cursor-not-allowed"
									>
								...
								{:else}
									<button
										onclick={() => onEnrollment(clase.id!, true)}
										class="rounded bg-green-50 px-2 py-1 text-xs text-green-600 hover:bg-green-100 hover:text-green-900"
									>
										Inscribirse
									</button>
								{/if}
							...
							{#if showTeacherActions && isTeacher(clase)}
								<button
									onclick={() => goto(`/clases/${clase.id}`)}
									class="rounded bg-blue-50 px-2 py-1 text-xs text-blue-600 hover:bg-blue-100 hover:text-blue-900"
								>
									Gestionar
								</button>
							{/if}
